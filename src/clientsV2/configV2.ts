...
configV2.validate({ allowed: 'strict' });

// Use environment variable for tip accounts, fallback to hardcoded list
const TIP_ACCOUNTS_ENV = process.env.TIP_ACCOUNTS;
// ✅ V2 approach
const TIP_ACCOUNTS: Address[] = TIP_ACCOUNTS_ENV 
  ? TIP_ACCOUNTS_ENV.split(',').map((pubkey) => address(pubkey.trim()))
  : [
...
const getRandomTipAccount = (): Address =>
  TIP_ACCOUNTS[Math.floor(Math.random() * TIP_ACCOUNTS.length)];

export { configV2, getRandomTipAccount };